...
export function NavyRankManager() {
  ...
  const handleImageUpload = async (file: File, chevronId: string) => {
    try {
      setUploading(chevronId)

      const fileExt = file.name.split('.').pop()
      const fileName = `${chevronId}.${fileExt}`
  ...
  const handleDescriptionUpdate = async (chevronId: string, description: string) => {
    try {
  ...
  const toggleActive = async (chevronId: string, currentActive: boolean) => {
    try {
      const { error } = await supabase
        .from('navy_ranks')
